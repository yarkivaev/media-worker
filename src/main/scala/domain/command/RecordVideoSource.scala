...
case class RecordVideoSource(source: MediaSource, mediaSink: MediaSink) extends MediaWorkerCommand {
...
object RecordVideoSource {
  given Encoder[RecordVideoSource] = (rv: RecordVideoSource) =>
    Json.obj(
      "source" -> rv.source.asJson,
      "mediaSink" -> rv.mediaSink.asJson
    )

  given Decoder[RecordVideoSource] = (c: HCursor) =>
    for {
